...
import { actualizarPersona, crearPersona } from "../../../redux/actions/Persona";
...
const PersonaForm = ({ editar = false, dataForm, entidad, mantenimiento }) => {

  const state = useSelector((state) => state);
  const { personaReducer, maestraReducer } = state;
  const { messageCrud } = personaReducer;
  const { loadingTipoDocumento,
    tipoDocumentoList,
    messageTipoDocumento,
    loadingSexo,
    sexoList,
  ...
  useEffect(() => {
    setTipoDocumentos(tipoDocumentoList)
  }, [tipoDocumentoList]);
  ...
  const ejecutar = (valores, resetForm) => {
    ...
    if (editar) {
      dispatch(actualizarPersona({ ...valores, 'stakeholder': mantenimiento.codigo } ));
    } else {
      dispatch(crearPersona({ ...valores, 'stakeholder': entidad.stakeholder, 'negocio': entidad.negocio, 'sistema': entidad.sistema },resetForm, navigate ));
    }
          ...
          <form onSubmit={props.handleSubmit}>
            <Grid container spacing={8}>
              <Grid item xs={12} sm={4}>
                ...
                { tipoDocumentos && tipoDocumentos.map((item, index) => {
                  return (
                    <MenuItem
                      key={item.idMaestra}
                      value={item.idMaestra}
                    >
                      {item.nombre}
                    </MenuItem>
                  );
