...
const ServicioForm = ({ editar = false, dataForm }) => {
  const linkRedirect = `/list-servicio`;
  const state = useSelector((state) => state);
  const { servicioReducer } = state;
  const { loadingCrud, messageCrud } = servicioReducer;
  ...
  const ejecutar = (valores,resetForm) => {
    if (editar) {
      dispatch(
        actualizarServicio({ ...valores },resetForm,navigate)
      );
    } else {
      dispatch(
        crearServicio({ ...valores },resetForm,navigate)
      );
    }
