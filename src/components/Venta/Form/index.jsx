...
const VentaForm = ({ editar = false, dataForm }) => {
  const linkRedirect = `/list-venta`;
  const state = useSelector((state) => state);
  const { ventaReducer } = state;
  const { loadingCrud, messageCrud } = ventaReducer;
  ...
  const ejecutar = (valores,resetForm) => {
    if (editar) {
      dispatch(
        actualizarVenta({ ...valores },resetForm,navigate)
      );
    } else {
      dispatch(
        crearVenta({ ...valores },resetForm,navigate)
      );
    }
